...
select * from Subject s where s.Credit =(select Max(Credit) from Subject);
...
# 3.• Hiển thị các thông tin sinh viên và điểm trung bình của mỗi
# sinh viên, xếp hạng theo thứ tự điểm giảm dần

select s.StudentId,s.StudentName, AVG(m.Mark) `Điểm trung bình` from Student s
